...
@Client.on_message(command(["song","غنيه","حميل اغنيه","تحميل اغنيه","اغنيه", f"song@{BOT_USERNAME}"]) & ~filters.edited)
def song(_, message):
...
@Client.on_message(
    command(["video", "حميل فديو","تحميل فديو","تنزيل فديو", f"vsong@{BOT_USERNAME}", "video", f"video@{BOT_USERNAME}"]) & ~filters.edited
)
async def vsong(client, message):
    ...
    try:
        ...
        print(e)
    try:
        msg = await message.reply("قناه البوت: @{UPDATES_CHANNEL} ...")
